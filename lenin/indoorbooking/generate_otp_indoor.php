...
function generateBookingId($conn, $table_id, $booking_date) {
    ...
    $query = "SELECT COUNT(*) AS count FROM indoorbooking WHERE booking_date = ?";
    ...
    $booking_id = "VF-ITB-" . $formatted_date . "-" . $table_id . "-" . $padded_n;
...
if (sendVerificationEmail($userEmail, $otp)) { 
    ...
    header("Location: mailvarrification_indoor.php");
    exit();
} else { 
